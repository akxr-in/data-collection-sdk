...
export class EventQueue {
  ...
  enqueue(evt: QueuedEvent): void {
    if (this.events.length >= this.opts.maxQueueSize) {
      if (this.opts.debug)
        console.warn("[data-collection] queue full, dropping oldest");
      this.events.shift();
    }
  ...
  async flush(): Promise<void> {
    ...
    try {
      ...
      const { error, response } = await this.transport.client.POST(
        "/v1/events",
        { body },
      );
      if (error) throw new TransportError(response.status, error);
      if (this.opts.debug)
        console.log(`[data-collection] flushed ${batch.length} events`);
    } catch (err) {
      this.events.unshift(...batch);
      if (this.opts.onError) this.opts.onError(err as Error, batch);
      else if (this.opts.debug)
        console.error("[data-collection] flush failed", err);
    } finally {
      this.flushing = false;
    }
  }
  ...
  flushBeacon(): boolean {
    ...
    const batch = this.events.splice(0, this.opts.maxBatchSize);
    const ok = this.transport.beacon(
      "/v1/events",
      buildEnvelope(this.getContext(), batch),
    );
